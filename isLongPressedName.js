/*
Your friend is typing his name into a keyboard. 
Sometimes, when typing a character c, the key might get long pressed, and the character will be typed 1 or more times.

You examine the typed characters of the keyboard. 
Return True if it is possible that it was your friends name, with some characters (possibly none) being long pressed.

 

Example 1:
Input: name = "alex", typed = "aaleex"
Output: true
Explanation: 'a' and 'e' in 'alex' were long pressed.

Example 2:
Input: name = "saeed", typed = "ssaaedd"
Output: false
Explanation: 'e' must have been pressed twice, but it was not in the typed output.
 

Constraints:
1 <= name.length, typed.length <= 1000
name and typed consist of only lowercase English letters.
*/

/**
 * @param {string} name
 * @param {string} typed
 * @return {boolean}
 */

/*
function isLongPressedName with name, typed parameter:
let i = 0;
let j = 0;
while j < typed.length:
    if characters match:
        move both i and j
    else if typed[j] is a long press:
        move only j
    else:
        return false
return i == name.length
*/

var isLongPressedName = function(name, typed) {
    let i = 0;  // pointer for name
    let j = 0;  // pointer for typed
    
    while (j < typed.length) {
        if (i < name.length && name[i] === typed[j]) {
            // Case 1: characters match, move both pointers
            i++;
            j++;
        } else if (j > 0 && typed[j] === typed[j - 1]) {
            // Case 2: current typed char is a long press of previous
            j++;
        } else {
            // Case 3: invalid character
            return false;
        }
    }
    
    // Check if we've consumed all characters from name
    return i === name.length;
};

console.log(isLongPressedName("alex", "aaleex")); // true
console.log(isLongPressedName("saeed", "ssaaedd")); // false
